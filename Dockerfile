...
RUN set -x\
    && chown -R feed:feed /opt/feed \
    && cd /opt/feed \
    && sudo -u feed php composer.phar install --no-dev --prefer-dist \
...
ENV CENTRIFUGO_HOST null
ENV CENTRIFUGO_API_KEY apiKey
ENV CENTRIFUGO_SECRET_KEY secretKey
ENV CENTRIFUGO_MODULE module
ENV BADGES_HOST null
